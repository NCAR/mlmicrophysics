...
implicit none
contains
subroutine collect_step(deltatin, t, rho, qcin, ncin, qrin, nrin, &
    ...
    real(kind=r8) :: lcldm(mgncol)
    real(kind=r8) :: precip_frac(mgncol)

    real(kind=r8) :: scale_qc(mgncol)
    real(kind=r8) :: scale_qr(mgncol)
    real(kind=r8) :: scale_nc(mgncol)
    real(kind=r8) :: scale_nr(mgncol)
    real(kind=r8) :: amk_c(mgncol,ncd)
    real(kind=r8) :: ank_c(mgncol,ncd)
    real(kind=r8) :: amk_r(mgncol,ncd)
    real(kind=r8) :: ank_r(mgncol,ncd)
    real(kind=r8) :: amk(mgncol,ncd)
    real(kind=r8) :: ank(mgncol,ncd)
    real(kind=r8) :: amk_out(mgncol,ncd)
    real(kind=r8) :: ank_out(mgncol,ncd)
