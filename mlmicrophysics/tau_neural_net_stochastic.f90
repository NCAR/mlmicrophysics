module tau_neural_net
    ...
    contains
        ...
        subroutine discrete_random_class(probabilities, n_classes, class_index)
            integer, intent(in) :: n_classes
            real(r8), intent(in) :: probabilities(n_classes)
            integer, intent(out) :: class_index
            real(r8) :: rand_selector, prob_sum
            integer :: i
            call random_number(rand_selector)
            prob_sum = 0
            class_index = 1
            do i=1, n_classes
                prob_sum = prob_sum + probabilities(i)
                if (prob_sum > rand_selector) then
                    class_index = i
                    exit
                end if
            end do
        end subroutine discrete_random_class
